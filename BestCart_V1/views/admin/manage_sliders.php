...
if (isset($_POST['add_slider'])) {
    ...
    if (addSlider($title, $subtitle, $image)) {
        echo "<script>alert('Slider Added!'); window.location.href='manage_sliders.php';</script>";
    } else {
...
if (isset($_GET['delete'])) {
    deleteSlider($_GET['delete']);
    echo "<script>window.location.href='manage_sliders.php';</script>";
}
                ...
                <tbody>
                    <?php foreach ($sliders as $s) { ?>
                        <tr>
                            ...
                            <td>
                                <a href="manage_sliders.php?delete=<?= $s['id'] ?>" class="btn btn-danger" style="padding:5px 10px; font-size:0.85rem;" onclick="return confirm('Delete this slide?')">
